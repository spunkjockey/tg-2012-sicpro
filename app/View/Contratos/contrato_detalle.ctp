	...
	<div id="Proyecto">
		...
		<tr>
			<?php if($contratos['Contrato']['tipocontrato']=='Construcción de obras') { ?>
				<td class="primerac">Monto Final:</td><td><?php echo '$' . number_format($contratos['Contrato']['montooriginal']+$contratosc['Contratoconstructor']['variacion'],2)?></td>
			<?php } ?>
		</tr>
		<tr>
		...
		<thead>
		<tr>
		   <th data-field="Titulo" width="40%">Titulo</th>
		   <th data-field="Monto" width="15%">Monto</th>
		...
		<tr>
			...
			<td><?php echo '$' . number_format($oc['Ordendecambio']['montoordencambio'],2)?></td>
			<td><?php echo $oc['Ordendecambio']['descripcionordencambio']?></td>
			<td><?php echo date('d/m/Y',strtotime($oc['Ordendecambio']['fecharegistroorden']))?></td>
		</tr>
		<?php endforeach ?>
		</tbody>
		</table>
